...
def _normalize_guidance_mode(guidance_mode: str) -> str:
    ...
    aliases = {
        ...
        "operators_off": "operator_off",
        "a_off": "operator_off",
        "b_only": "operator_off",
        "ab_operator_off": "operator_off",
    }
    mode = aliases.get(mode, mode)
    if mode not in {"price", "graph", "off", "operator_off"}:
        raise ValueError(
            f"Unknown guidance_mode={guidance_mode!r}. "
            "Use 'price', 'graph', 'off', or 'operator_off'."
        )
    return mode
    ...
    def summary_rows(self):
        ...
        for rank, item in enumerate(sorted(self.items, key=self._combined_sort_key), start=1):
            ...
            rows.append({
                ...
                "pressure_cost": m.get("pressure_cost"),
...
def _candidate_score_for_insertion_no_pressure(row: pd.Series) -> float:
    return (
        5.0 * _safe_float(row.get("or_overtime_min", 0.0))
        + 2.0 * _safe_float(row.get("calendar_outside_min", 0.0))
        + 0.01 * _safe_float(row.get("duration_min", 0.0))
...
def _choose_best_candidate_no_pressure(placements: pd.DataFrame, pid: int, rng: random.Random, top_k: int = 8) -> Optional[int]:
    cand = placements[placements["patient_id"].astype(int) == int(pid)].copy()
    if cand.empty:
        return None
    score = cand.apply(_candidate_score_for_insertion_no_pressure, axis=1)
    idxs = score.nsmallest(min(top_k, len(score))).index.tolist()
    return int(rng.choice(idxs))


def _choose_relocation_candidate_no_pressure(placements: pd.DataFrame,
                                             pid: int,
                                             current_j: int,
                                             rng: random.Random,
                                             radius: int) -> Optional[int]:
...
def _removal_score_no_pressure(pid: int, j: int, placements: pd.DataFrame,
                               pclass: Dict[int, str], high_ids: set[int], rng: random.Random) -> float:
    r = placements.loc[int(j)]
    cls = pclass.get(pid, "medium")
    rank = {"low": 0, "medium": 1, "high": 3}.get(cls, 1)
    duration = _safe_float(r.get("duration_min", 0.0), 0.0)
    overtime = _safe_float(r.get("or_overtime_min", 0.0), 0.0)
    return -5000.0 * rank + 0.03 * duration + 0.05 * overtime + rng.random()


def _choose_removal_no_pressure(selected: Dict[int, int], placements: pd.DataFrame,
                                pclass: Dict[int, str], high_ids: set[int],
                                rng: random.Random, allow_high: bool = False) -> Optional[int]:
    cand = []
    for pid, j in selected.items():
        if pid in high_ids and not allow_high:
            continue
        cand.append((_removal_score_no_pressure(pid, j, placements, pclass, high_ids, rng), pid))
    if not cand:
        return None
    cand.sort(reverse=True)
    return int(cand[0][1])
...
def _propose_neighbor_operator_off(selected: Dict[int, int],
                                   placements: pd.DataFrame,
                                   pclass: Dict[int, str],
                                   high_ids: set[int],
                                   target_volume: int,
                                   rng: random.Random,
                                   radius: int,
                                   phase: str) -> Tuple[Dict[int, int], str]:
    """A-off neighborhood: no pressure-guided destroy/repair choices.

    The pressure column may still exist and may still be used by the scalar fast
    objective through RECOURSE_PRESSURE_WEIGHT. This is the B-only arm.
    """
    new = dict(selected)
    ...
    if phase == "overtime":
        probs = {"overtime_relocate": 0.45, "insert_high": 0.20, "replace": 0.20, "multi_shift": 0.15}
    else:
        probs = {"overtime_relocate": 0.30, "insert_high": 0.25, "replace": 0.25, "multi_shift": 0.20}
    moves = list(probs.keys())
    weights = [probs[m] for m in moves]
    move = rng.choices(moves, weights=weights, k=1)[0]

    if move == "overtime_relocate" and new:
        overtime_pids = []
        ...
        if overtime_pids:
            ...
            jj = _choose_relocation_candidate_no_pressure(placements, pid, new[pid], rng, radius)
            if jj is not None:
                new[pid] = jj
        ...
        if missing_high:
            ...
            j_add = _choose_best_candidate_no_pressure(placements, pid_add, rng)
            if j_add is not None:
                if len(new) >= target_volume:
                    pid_rm = _choose_removal_no_pressure(new, placements, pclass, high_ids, rng, allow_high=False)
...
def _spiral_perturb_operator_off(selected: Dict[int, int],
                                 placements: pd.DataFrame,
                                 pclass: Dict[int, str],
                                 high_ids: set[int],
                                 target_volume: int,
    ...
    k_remove = max(1, int(round(remove_frac * len(new)))) if new else 0
    removable = [pid for pid in new if pid not in high_ids]
    removable.sort(key=lambda pid: _removal_score_no_pressure(pid, new[pid], placements, pclass, high_ids, rng), reverse=True)
    ...
    for pid in fill:
        if len(new) >= target_volume:
            break
        jj = _choose_best_candidate_no_pressure(placements, int(pid), rng)
        if jj is not None:
            new[int(pid)] = jj
    return new
...
def propose_neighbor(selected: Dict[int, int],
    ...
    if guidance_mode == "operator_off":
        return _propose_neighbor_operator_off(
            selected, placements, pclass, high_ids, target_volume, rng, radius, phase
        )
...
def run_spiral_pr_glns(instance: Dict[str, Any],
    ...
    # Recourse-price calibration patch.
    # If RECOURSE_PRESSURE_WEIGHT is set by the outer Level-1 runner, use it
    # for the fast surrogate objective under price/graph/off modes.  Without
    # this, price_cal_w0/w1 silently collapse to the hard-coded 1500.0 weight
    # whenever guidance_mode="price".
    raw_pressure_weight = os.environ.get("RECOURSE_PRESSURE_WEIGHT", "").strip()
    if raw_pressure_weight != "":
        try:
            recourse_pressure_weight_fast = float(raw_pressure_weight)
        except Exception:
            raise ValueError(f"Invalid RECOURSE_PRESSURE_WEIGHT={raw_pressure_weight!r}")
    elif guidance_mode == "off":
        recourse_pressure_weight_fast = 0.0
    else:
        recourse_pressure_weight_fast = 1_500.0
    ...
    weights_fast = {
        ...
        "ward_excess": 1_000.0,
        "peak_blocked": 550.0,
        "pressure": recourse_pressure_weight_fast,
    }
    weights_exact = {
    ...
    for cyc in range(1, cycles + 1):
        ...
        for k in range(1, proposals_per_cycle + 1):
            ...
            if k % max(10, exact_every // 2) == 0:
                if guidance_mode == "operator_off":
                    cand_selected = _spiral_perturb_operator_off(base_selected, placements, pclass, high_ids, target_volume, rng, radius)
                    move = "spiral_perturb_operator_off"
                else:
            ...
            if iteration % 10 == 0 or added:
                ...
                trace.append({
                    ...
                    "best_stage3_ward_excess": bs.get("ward_excess_bed_days_stage3"),
                    "best_stage3_peak": bs.get("peak_icu_ready_blocked_stage3"),
                    "best_pressure_cost": bm.get("pressure_cost"),
                    "target_volume": int(target_volume),
                    "high_target": int(high_target),
    ...
    meta = {
        "seed": seed,
        "guidance_mode": guidance_mode,
        "operator_mode": "off" if guidance_mode in {"off", "operator_off"} else "on",
        "recourse_pressure_weight_fast": recourse_pressure_weight_fast,
        ...
        "recourse_pressure_weight_fast": recourse_pressure_weight_fast,
        "recourse_pricing_arm": os.environ.get("RECOURSE_PRICING_ARM", ""),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--guidance-mode", choices=["price", "graph", "off", "operator_off"], default="price",
                        help="price: A on; graph: price + graph destroy; off: A/B off and pressure zeroed; operator_off: A off but pressure costs retained for B-only.")
